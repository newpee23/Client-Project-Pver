...
// ดึงข้อมูล Page0F1 
export const findPage0F1 = createAsyncThunk<{ message: Opprovince[] | string, status: boolean }, string>(
  "findPage0F1/loadAsync", async (fId: string): Promise<{ message: Opprovince[] | string, status: boolean }> => {
    try {
      const response: AxiosResponse = await axios.get(
      ...
      return response.data as { message: Opprovince[], status: boolean };
    } catch (error: unknown) {
      if (axios.isAxiosError(error)) {
        const axiosError = error as AxiosError;
        if (axiosError.response && axiosError.response.status === 401) {
...
const pageSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    // findPage0F1
    builder.addCase(findPage0F1.fulfilled, (state, action) => {
      if (typeof action.payload.message !== 'string') {
        state.page1.message = action.payload.message;
        state.page1.status = action.payload.status;
      }
    });
    builder.addCase(findPage0F1.rejected, (state, action) => {
      state.message = action.error.message || "";
    });
  },
});
